...
class QuoteTest extends TestCase
{
    ...
    /**
     * SCRUM-106: duplicate configurable children with the same product_id are merged after updateItem().
     *
     * @return void
     */
    public function testUpdateItemMergesDuplicateChildItemsByProductId(): void
    {
        ...
        $quoteItemMock = $this->createPartialMock(Item::class, ['getProduct', 'getBuyRequest']);
        $quoteItemMock->method('getProduct')->willReturn($productMock);
        $quoteItemMock->method('getBuyRequest')->willReturn(new DataObject());

        $childExisting = $this->createPartialMock(Item::class, ['getProductId', 'getId', 'getQty', 'setQty']);
        $childExisting->method('getProductId')->willReturn(501);
        $childExisting->method('getId')->willReturn(201);
        $childExisting->method('getQty')->willReturn(1.0);
        $childExisting->expects($this->once())->method('setQty')->with(3.0);

        $childDuplicate = $this->createPartialMock(Item::class, ['getProductId', 'getId', 'getQty']);
        $childDuplicate->method('getProductId')->willReturn(501);
        $childDuplicate->method('getId')->willReturn(202);
        $childDuplicate->method('getQty')->willReturn(2.0);

        $resultItem = $this->createPartialMock(
            Item::class,
            ['getId', 'getParentItem', 'setQty', 'getChildren']
        );
        $resultItem->method('getId')->willReturn($itemId);
        $resultItem->method('getParentItem')->willReturn(null);
        $resultItem->expects($this->once())->method('setQty')->with(1);
        $resultItem->method('getChildren')->willReturn([$childExisting, $childDuplicate]);

        $quote = $this->createUpdateItemQuoteMock(
            $itemId,
            $quoteItemMock,
            $productMock
        );
        ...
        $buyRequest = new DataObject(['qty' => 1, 'id' => $itemId]);
        $this->assertSame($resultItem, $quote->updateItem($itemId, $buyRequest));
    }

    /**
     * SCRUM-106: when buy request has no id, updateItem() sets it to the quote item id before addProduct().
     *
     * @return void
     */
    public function testUpdateItemSetsBuyRequestIdWhenMissing(): void
    {
        ...
        $resultItem->method('getId')->willReturn($itemId);
        $resultItem->method('getParentItem')->willReturn(null);
        $resultItem->expects($this->once())->method('setQty');
        ...
        $quote->expects($this->once())
            ...
            ->with(
                $this->anything(),
                $this->callback(static function (DataObject $buyRequest) use ($itemId) {
                    return $buyRequest->getId() === $itemId && $buyRequest->getQty() == 2;
                })
            )
    ...
    /**
     * @param int $itemId
     * @param Item|MockObject $quoteItemMock
     * @param Product|MockObject $productMock
     * @return Quote|MockObject
     */
    private function createUpdateItemQuoteMock(
        int $itemId,
        MockObject $quoteItemMock,
        ...
        $catalogProductMock->method('addParamsToBuyRequest')
            ->willReturnCallback(static function ($buyRequest, $params) {
                return $buyRequest instanceof DataObject ? $buyRequest : new DataObject();
            });

        $productFromRepo = $this->createMock(Product::class);
        $productFromRepo->method('getId')->willReturn($productMock->getId());
        $productRepositoryMock = $this->createMock(ProductRepositoryInterface::class);
        $productRepositoryMock->method('getById')->willReturn($productFromRepo);

        $quote = $this->getMockBuilder(Quote::class)
            ->disableOriginalConstructor()
            ->onlyMethods(['getItemById', 'addProduct', 'removeItem', 'getStore'])
            ->getMock();
        $quote->method('getItemById')->with($itemId)->willReturn($quoteItemMock);
        $quote->method('getStore')->willReturn($storeMock);

        $reflection = new \ReflectionClass(Quote::class);
        $catalogProperty = $reflection->getProperty('_catalogProduct');
        $catalogProperty->setAccessible(true);
        $catalogProperty->setValue($quote, $catalogProductMock);
        $repositoryProperty = $reflection->getProperty('productRepository');
        $repositoryProperty->setAccessible(true);
        $repositoryProperty->setValue($quote, $productRepositoryMock);
